//-----------------------------------------------------------
// Shyann Francis
// DDG Reader "driver" program
// DUCKDUCKGOOSEParser.cpp
//-----------------------------------------------------------
#include <iostream>
#include <iomanip>
...
//--------------------------------------------------
{
    ...
    lister.ListInformationLine(information);
    lister.ListInformationLine("DDG compiler ending with compiler error!\n");
    throw(DDGEXCEPTION("DDG compiler ending with compiler error!"));
}
...
//-----------------------------------------------------------
{
    ...
    void ParseDDGProgram(TOKEN tokens[]);
    ...
    try
    {
        ...
        ParseDDGProgram(tokens);
    }
    catch (DDGEXCEPTION Exception)
    {
        cout << "DDG exception: " << Exception.GetDescription() << endl;
    }
    lister.ListInformationLine("******* DDG parser ending");
    cout << "DDG parser ending\n";
...
//-----------------------------------------------------------
{
    ...
    while (tokens[0].type != EndFunction)
...
//-----------------------------------------------------------
{
    ...
    // if ((nextCharacter == '*') && (reader.GetLookAheadCharacter(1).character == '*') && (reader.GetLookAheadCharacter(2).character == '*') || ((nextCharacter == '\\') && (reader.GetLookAheadCharacter(1).character == '*')))
        ...
        } while ((nextCharacter == ' ') || (nextCharacter == READER<CALLBACKSUSED>::EOLC) || (nextCharacter == READER<CALLBACKSUSED>::TABC) || (nextCharacter == '*') && (reader.GetLookAheadCharacter(1).character == '*') && (reader.GetLookAheadCharacter(2).character == '*') || ((nextCharacter == '\\') && (reader.GetLookAheadCharacter(1).character == '*')));
    //============================================================
    // Scan token
    //============================================================
